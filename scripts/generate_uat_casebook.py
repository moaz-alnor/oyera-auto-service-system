"""Generate exact manual UAT procedures for all OYERA roles."""
...
OUTPUT_PATH = Path("docs/uat/role-uat-casebook.md")
...
def _url(
    ...
    **kwargs: int,
    ...
    """Resolve one named application URL."""
    return reverse(
        name,
...
def _render_markdown(text: str) -> str:
    """Normalize table spacing in the generated document."""
...
def main() -> None:
    """Resolve seeded records and create the manual UAT casebook."""
    ...
    inventory_item = InventoryItem.objects.select_related(
        "product",
        "location",
    ).get(product__sku="OIL-FILTER-001")

    submitted_purchase_order = PurchaseOrder.objects.get(
        supplier_reference="DEMO-PO-SUBMITTED",
    )

    unpaid_supplier_invoice = SupplierInvoice.objects.get(
    ...
    def work_order_for(
        ...
        return WorkOrder.objects.select_related(
            "job_card",
            "job_card__vehicle",
        ).get(job_card__vehicle__registration_number=(registration_number))

    reserve_order = work_order_for("UAT 101A")
    issue_order = work_order_for("UAT 202B")
    return_order = work_order_for("UAT 303C")
    paid_release_order = work_order_for("UAT 404D")
    override_release_order = work_order_for("UAT 505E")

    reserve_requirement = reserve_order.product_requirements.get()
    issue_requirement = issue_order.product_requirements.get()

    issue_reservation = StockReservation.objects.get(
        work_product_requirement=issue_requirement,
    )

    return_issue_movement = StockMovement.objects.get(
        movement_type=StockMovementType.ISSUE,
        quantity=Decimal("1.500"),
        notes__icontains="return testing",
    )

    issue_task = issue_order.tasks.get()
    return_task = return_order.tasks.get()

    paid_invoice = Invoice.objects.get(
        work_order=paid_release_order,
    )
    unpaid_invoice = Invoice.objects.get(
        work_order=override_release_order,
    )

    customer_create_url = _url(
        "customers:create",
    )
    vehicle_create_url = _url(
        "vehicles:create",
    )
    job_create_url = _url(
        "jobs:create",
    ...
    purchase_order_approve_url = _url(
        "purchasing:purchase_order_approve",
        purchase_order_id=submitted_purchase_order.pk,
    )

    reserve_url = _url(
        "inventory:reserve",
        requirement_id=reserve_requirement.pk,
    )
    issue_url = _url(
        "inventory:issue",
        reservation_id=issue_reservation.pk,
    )
    return_url = _url(
        "inventory:return",
        movement_id=return_issue_movement.pk,
    )

    paid_release_url = _url(
        "jobs:release_create",
        job_card_id=paid_release_order.job_card_id,
    )
    override_release_url = _url(
        "jobs:release_create",
        job_card_id=override_release_order.job_card_id,
    )

    customer_payment_url = _url(
        "billing:payment_create",
        invoice_id=unpaid_invoice.pk,
    )
    supplier_payment_url = _url(
        "purchasing:supplier_payment_record",
        supplier_invoice_id=unpaid_supplier_invoice.pk,
    )

    issue_order_start_url = _url(
        "workshop:start",
        work_order_id=issue_order.pk,
    )
    return_order_start_url = _url(
        "workshop:start",
        work_order_id=return_order.pk,
    )
    issue_order_hold_url = _url(
        "workshop:hold",
        work_order_id=issue_order.pk,
    )
    issue_order_resume_url = _url(
        "workshop:resume",
        work_order_id=issue_order.pk,
    )

    issue_task_start_url = _url(
        "workshop:start_task",
        work_task_id=issue_task.pk,
    )
    issue_task_block_url = _url(
        "workshop:block_task",
        work_task_id=issue_task.pk,
    )
    issue_task_note_url = _url(
        "workshop:add_task_note",
        work_task_id=issue_task.pk,
    )

    return_task_start_url = _url(
        "workshop:start_task",
        work_task_id=return_task.pk,
    )
    return_task_review_url = _url(
        "workshop:submit_task_review",
        work_task_id=return_task.pk,
    )
    return_task_approve_url = _url(
        "workshop:approve_task",
        work_task_id=return_task.pk,
    )

    reports_index_url = _url(
        "reports:index",
    )
    customer_finance_report_url = _url(
        "reports:customer_finance",
    )
    customer_finance_export_url = _url(
        "reports:customer_finance_export",
    )
    workshop_report_url = _url(
        "reports:workshop_operations",
    )
    workshop_export_url = _url(
        "reports:workshop_operations_export",
    )
    inventory_report_url = _url(
        "reports:inventory_activity",
    )
    inventory_export_url = _url(
        "reports:inventory_activity_export",
    )
    purchasing_report_url = _url(
        "reports:purchasing_activity",
    )
    purchasing_export_url = _url(
        "reports:purchasing_activity_export",
    )

    paid_release_record = f"`UAT 404D` / invoice `{paid_invoice.pk}`"
    override_release_record = f"`UAT 505E` / invoice `{unpaid_invoice.pk}`"

    document = f"""# OYERA Role-by-Role User Acceptance Test Casebook
...
|---|---|---|
| Submitted purchase order | `DEMO-PO-SUBMITTED` | `{purchase_order_approve_url}` |
| Reserve stock | `UAT 101A` | `{reserve_url}` |
| Issue stock | `UAT 202B` | `{issue_url}` |
| Return stock | `UAT 303C` | `{return_url}` |
| Paid vehicle release | {paid_release_record} | `{paid_release_url}` |
| Unpaid override release | {override_release_record} | `{override_release_url}` |
| Customer payment | Unpaid invoice `{unpaid_invoice.pk}` | `{customer_payment_url}` |
| Supplier payment | `DEMO-SINV-UNPAID` | `{supplier_payment_url}` |
| Assigned task A | Task `{issue_task.pk}` | `{issue_task_start_url}` |
| Assigned task B | Task `{return_task.pk}` | `{return_task_start_url}` |
...
1. Open `{BASE_URL}/accounts/login/`.
2. Enter `admin`.
...
- `{customer_finance_report_url}`
- `{workshop_report_url}`
- `{inventory_report_url}`
- `{purchasing_report_url}`

Then download:

- `{customer_finance_export_url}`
- `{workshop_export_url}`
- `{inventory_export_url}`
- `{purchasing_export_url}`
...
1. Open `{purchase_order_approve_url}`.
...
3. Select:
   `{inventory_item.product.sku} - {inventory_item.product.name} -
...
1. Log in as `admin`.
2. Open `{issue_order_start_url}` and start the order.
3. Open `{return_order_start_url}` and start the order.
4. Log out.
...
1. Open `{issue_task_start_url}`.
2. Start task `{issue_task.pk}`.
3. Confirm status `IN_PROGRESS`.
4. Open `{issue_task_block_url}`.
5. Enter:
...
1. Open `{return_task_start_url}`.
2. Start task `{return_task.pk}`.
3. Open `{return_task_review_url}`.
...
1. Open `{return_task_approve_url}`.
